...
}(function ($, moment)
{
	...
	$.fn.dateRangePicker = function(opt)
	{
		...
		function updateCalendarWidth()
		{
            ...
            //扩展移动端
			var w4="100%";
			box.find('.month-wrapper').width(w4);
		}
		...
		function showSelectedInfo(forceValid,silent)
		{
			...
			else if (opt.start && opt.end)
			{
				...
				$("#num").html(countDays(opt.end, opt.start));
				box.find('.apply-btn').removeClass('disabled');
		...
		function countDays(start,end)
		{
			...
			return Math.abs( daysFrom1970(start) - daysFrom1970(end) ) + 0;
		...
		function showSelectedDays()
		{
			...
			box.find('.day').each(function()
			{
				...
				{
					$(this).addClass('first-date-selected').find("span").text("开始");
				}
				else
				{
					$(this).removeClass('first-date-selected').find("span").text(moment(time).format('D'));
				}
				//add last-date-selected
				if ( opt.end && moment(end).format('YYYY-MM-DD') == moment(time).format('YYYY-MM-DD') )
				{
					$(this).addClass('last-date-selected').find("span").text("结束");
				}
				else
				{
		...
		function showGap()
		{
			...
			{  
				...
				box.addClass('has-gap').removeClass('no-gap').find('.gap').css({visibility:'visible',opacity:0});
			}
			else
			{
				// box.removeClass('has-gap').addClass('no-gap').find('.gap').css('visibility','visible');
				box.removeClass('has-gap').addClass('no-gap').find('.gap').css({visibility:'visible',opacity:0});
			}
			var h1 = box.find('table.month1').height();
			var h2 = box.find('table.month2').height();
			// box.find('.gap').height(Math.max(h1,h2)+10);
			box.find('.gap').height(20);
		}
			...
			if (opt.showTopbar)
			{
				...
				else
				{
					html += '<div class="normal-top">' +
							'<span style="color:#333">'+lang('selected')+' </span>';
					if ( ! opt.singleDate ) {
						html += ' <i class="selected-days"><span class="selected-days-num">3</span> '+lang('days')+'</i>';
					}
					html += '</div>';
